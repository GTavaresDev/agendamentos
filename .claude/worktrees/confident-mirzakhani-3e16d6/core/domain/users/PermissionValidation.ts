...
import { UserPermissionProps } from './UserPermission';
...
export function hasPermission(
  ...
  const isRolePermission = ['Administrador', 'Gestor', 'Funcionario'].includes(permissionName);

  if (isRolePermission) {
    return context.role === permissionName;
  }

  return context.permissions?.some(p => p.name === permissionName && p.name !== context.role) ?? false;
}
