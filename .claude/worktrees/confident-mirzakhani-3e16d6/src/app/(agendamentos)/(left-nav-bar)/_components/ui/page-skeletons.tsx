        ...
        <Card className="min-h-[420px]">
          ...
          <CardContent className="space-y-0 px-0 pb-2">
            {Array.from({ length: 5 }).map((_, index) => (
              <div
                key={index}
                className="flex h-[68px] items-center gap-4 border-t border-zinc-100 px-5"
              >
                <Skeleton className="h-4 w-12" />
                <Skeleton className="h-9 w-0.5" />
      ...
      <Card className="overflow-hidden">
        ...
        <div className={TABLE_BODY_MIN_HEIGHT_CLASS}>
          {Array.from({ length: rows }).map((_, rowIndex) => (
            <div
              key={rowIndex}
              className={`flex ${TABLE_ROW_MIN_HEIGHT_CLASS} items-center gap-4 border-b border-zinc-100 px-5`}
            >
        ...
        <div className="flex h-[60px] items-center justify-between border-t border-zinc-100 px-5">
          <Skeleton className="h-4 w-40" />
      ...
      <Card className="overflow-hidden">
        ...
        <div className={TABLE_BODY_MIN_HEIGHT_CLASS}>
          {Array.from({ length: 8 }).map((_, index) => (
            <div
              key={index}
              className={`flex ${TABLE_ROW_MIN_HEIGHT_CLASS} items-center gap-4 border-b border-zinc-100 px-4 sm:px-6`}
            >
...
export function SchedulingSkeleton() {
  return (
    <div className="mx-auto max-w-[1500px] p-4 md:p-5 xl:p-8">
      <div className="mb-5 flex min-h-[56px] flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_clamp(260px,30%,380px)] xl:gap-5">
        <div className="min-w-0 space-y-4 xl:space-y-5">
            ...
            <CardContent className="p-3 md:p-4 xl:p-5">
              <div className="grid grid-cols-7 gap-1.5 xl:gap-2">
                {Array.from({ length: 7 }).map((_, index) => (
                  <Skeleton
                    key={index}
                    className="min-h-16 rounded-xl md:min-h-20 xl:min-h-24"
                  />
                ))}
              </div>
            </CardContent>
          ...
          <Card className="min-h-[420px]">
            <CardHeader className="space-y-2 p-4 xl:p-5">
              <Skeleton className="h-5 w-44" />
              <Skeleton className="h-4 w-56" />
            </CardHeader>
            <CardContent className="space-y-6 p-4 pt-0 xl:p-5 xl:pt-0">
        ...
        <Card className="h-fit min-h-[560px] min-w-0 md:sticky md:top-[104px]">
          <CardHeader className="space-y-3 border-b border-zinc-100 p-4 xl:p-5">
